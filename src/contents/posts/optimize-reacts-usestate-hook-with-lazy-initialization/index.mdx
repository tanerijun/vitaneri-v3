...
As you know, reading from local storage is an IO operation, and it can potentially slow our application.

In the example above, the `useState` hook read the value from local storage every time it re-renders.

Is there a way we can optimize this? This is where **lazy initialization** come in to play.
