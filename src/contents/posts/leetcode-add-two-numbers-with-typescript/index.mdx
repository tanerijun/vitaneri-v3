...
import NormalAdditionImg from "./images/normal-addition.jpeg";
import ReversedAdditionImg from "./images/reversed-addition.jpg";
...
The time complexity is `O(Max(m, n))` because both of the list will only be traversed one, and the complexity depends on which list is longer.
...
That's it for LeetCode's "Add Two Numbers" :tada:.
